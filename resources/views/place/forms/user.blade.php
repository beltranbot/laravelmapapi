<h5>INFORMACIÓN GENERAL DEL ASEGURADO</h5>
<div class="form-row">
    <div class="form-group col-md-6">
        
        <label for="razon_comercial">Razón Comercial</label>
        <input
            class="form-control input-sm"
            style="width:100%"
            type="text"
            name="razon_comercial"
            {{isset($disabled) ? 'disabled' : ''}}
        >
    </div>
    <div class="form-group col-md-6">

        <label for="ruc">RUC</label>
        <input
            class="form-control input-sm"
            style="width:100%"
            type="text"
            name="ruc"
            {{isset($disabled) ? 'disabled' : ''}}
        >
    </div>
</div>
<div class="form-row">
    <div class="form-group col-md-8">
        
        <label for="direccion_principal">DIRECCIÓN PRINCIPAL</label>
        <input
            class="form-control input-sm"
            style="width:100%"
            type="text"
            name="direccion_principal"
            {{isset($disabled) ? 'disabled' : ''}}
        >
    </div>
    <div class="form-group col-md-4">

        <label for="numero">NUMERO</label>
        <input
            class="form-control input-sm"
            style="width:100%"
            type="text"
            name="numero"
            {{isset($disabled) ? 'disabled' : ''}}
        >
    </div>
</div>
<div class="form-row">
    <div class="form-group col-md-6">
        
        <label for="interseccion">INTERSECCIÓN</label>
        <input
            class="form-control input-sm"
            style="width:100%"
            type="text"
            name="interseccion"
            {{isset($disabled) ? 'disabled' : ''}}
        >
    </div>
    <div class="form-group col-md-3">

        <label for="edificio">EDIFICIO</label>
        <input
            class="form-control input-sm"
            style="width:100%"
            type="text"
            name="edificio"
            {{isset($disabled) ? 'disabled' : ''}}
        >
    </div>
    <div class="form-group col-md-3">

        <label for="piso">PISO</label>
        <input
            class="form-control input-sm"
            style="width:100%"
            type="text"
            name="piso"
            {{isset($disabled) ? 'disabled' : ''}}
        >
    </div>
</div>
<div class="form-row">
    <div class="form-group col-md-6">
        
        <label for="telefono1">TELÉFONO 1</label>
        <input
            class="form-control input-sm"
            style="width:100%"
            type="text"
            name="telefono1"
            {{isset($disabled) ? 'disabled' : ''}}
        >
    </div>
    <div class="form-group col-md-6">

        <label for="telefono2">TELÉFONO2</label>
        <input
            class="form-control input-sm"
            style="width:100%"
            type="text"
            name="telefono2"
            {{isset($disabled) ? 'disabled' : ''}}
        >
    </div>
</div>
<div class="form-row">
    <div class="form-group col-md-6">
        
        <label for="telefono3">TELÉFONO 3</label>
        <input
            class="form-control input-sm"
            style="width:100%"
            type="text"
            name="telefono3"
            {{isset($disabled) ? 'disabled' : ''}}
        >
    </div>
    <div class="form-group col-md-6">

        <label for="telefax">TELÉFAX</label>
        <input
            class="form-control input-sm"
            style="width:100%"
            type="text"
            name="telefax"
            {{isset($disabled) ? 'disabled' : ''}}
        >
    </div>
</div>
<div class="form-row">
    <div class="form-group col-md-3">
        
        <label for="numero_sucursales">NÚMERO DE SUCURSALES</label>
        <input
            class="form-control input-sm"
            style="width:100%"
            type="text"
            name="numero_sucursales"
            {{isset($disabled) ? 'disabled' : ''}}
        >
    </div>
    <div class="form-group col-md-9">

        <label for="correo_electronico">CORREO ELECTRÓNICO</label>
        <input
            class="form-control input-sm"
            style="width:100%"
            type="text"
            name="correo_electronico"
            {{isset($disabled) ? 'disabled' : ''}}
        >
    </div>
</div>
<div class="form-row">
    <div class="form-group col-md-6">
        
        <label for="solicitado_por">SOLICITADO POR</label>
        <input
            class="form-control input-sm"
            style="width:100%"
            type="text"
            name="solicitado_por"
            {{isset($disabled) ? 'disabled' : ''}}
        >
    </div>
    <div class="form-group col-md-6">

        <label for="aseguradora">ASEGURADORA</label>
        <input
            class="form-control input-sm"
            style="width:100%"
            type="text"
            name="aseguradora"
            {{isset($disabled) ? 'disabled' : ''}}
        >
    </div>
</div>
<div class="form-row">
    <div class="form-group col-md-12">        
        <label for="atendio">ATENDIÓ</label>
        <input
            class="form-control input-sm"
            style="width:100%"
            type="text"
            name="atendio"
            {{isset($disabled) ? 'disabled' : ''}}
        >
    </div>
</div>
<div class="form-row">
    <div class="form-group col-md-3">        
        <label for="fecha_asignacion">FECHA ASIGNACIÓN</label>
        <input
            class="form-control input-sm"
            style="width:100%"
            type="text"
            name="fecha_asignacion"
            {{isset($disabled) ? 'disabled' : ''}}
        >
    </div>
    <div class="form-group col-md-3">        
        <label for="fecha_inspeccion">FECHA INSPECCIÓN</label>
        <input
            class="form-control input-sm"
            style="width:100%"
            type="text"
            name="fecha_inspeccion"
            {{isset($disabled) ? 'disabled' : ''}}
        >
    </div>
    <div class="form-group col-md-3">        
        <label for="fecha_informe">FECHA INFORME</label>
        <input
            class="form-control input-sm"
            style="width:100%"
            type="text"
            name="fecha_informe"
            {{isset($disabled) ? 'disabled' : ''}}
        >
    </div>
    <div class="form-group col-md-3">
        <label for="inspector">INSPECTOR</label>
        <input
            class="form-control input-sm"
            style="width:100%"
            type="text"
            name="inspector"
            {{isset($disabled) ? 'disabled' : ''}}
        >
    </div>
</div>
<div class="form-row">
    <div class="form-group col-md-4">
        <label for="incendio_y_explosion">INCENDIO Y EXPLOSIÓN</label>
        <input
            class="form-control input-sm"
            style="width:100%"
            type="text"
            name="incendio_y_explosion"
            {{isset($disabled) ? 'disabled' : ''}}
        >
    </div>
    <div class="form-group col-md-4">
        <label for="lineas_aliadas">LÍNEAS ALIADAS</label>
        <input
            class="form-control input-sm"
            style="width:100%"
            type="text"
            name="lineas_aliadas"
            {{isset($disabled) ? 'disabled' : ''}}
        >
    </div>
    <div class="form-group col-md-4">
        <label for="robo_y_o_asalto">ROBO Y/O ASALTO</label>
        <input
            class="form-control input-sm"
            style="width:100%"
            type="text"
            name="robo_y_o_asalto"
            {{isset($disabled) ? 'disabled' : ''}}
        >
    </div>
</div>
<div class="form-row">
    <div class="form-group col-md-6">
        <label for="equipo_electronico">EQUIPO ELECTRONICO</label>
        <input
            class="form-control input-sm"
            style="width:100%"
            type="text"
            name="equipo_electronico"
            {{isset($disabled) ? 'disabled' : ''}}
        >
    </div>
    <div class="form-group col-md-6">
        <label for="responsabilida_civil">RESPONSABILIDA CIVIL</label>
        <input
            class="form-control input-sm"
            style="width:100%"
            type="text"
            name="responsabilidad_civil"
            {{isset($disabled) ? 'disabled' : ''}}
        >
    </div>
</div>